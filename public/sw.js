  ...
  if (
    ...
    if (event.request.url.includes("/api/auth")) {
      event.respondWith(
        fetch(event.request)
          .then((response) => {
            // If auth request fails (server restart), clear all caches
            if (!response.ok && response.status >= 500) {
              console.log("🔄 Server error detected, clearing caches");
          ...
          .catch((error) => {
            console.log("🔄 Network error detected, might be server restart");
